...
2.  事件冒泡
    ...
    ![事件冒泡.png](https://s2.loli.net/2022/08/23/TzK3dhf9uDiy28O.png) 3. 事件代理
...
3. 跨域 - 同源策略：ajax 请求时，浏览器要求当前网页和 server 同源（协议、域名、端口必须一致） - 图片(可以用于统计打点，使用第三方服务）、css、script（实现 JSONP） 无视同源策略 - 所有跨域都必须经过 server 端允许
4. JSONP
...
原则：多使用内存和缓存，减少 CPU 计算量，减少网络加载耗时，空间换时间
