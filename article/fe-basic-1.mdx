...
前端基础回顾是我用来准备前端基础面试（一面和二面部分）时写的笔记，随着面试次数增加不断更新。
...
>
    See the Pen <a href="https://codepen.io/starfurye/pen/zYjxaPa">圣杯布局</a>{" "}
...
>
    See the Pen{" "}
    <a href="https://codepen.io/starfurye/pen/yLjyERp">双飞翼布局</a> by Chenghe
    Hu (<a href="https://codepen.io/starfurye">@starfurye</a>) on{" "}
    <a href="https://codepen.io">CodePen</a>.
</iframe>

目的：中间栏先加载和渲染，两侧内容固定，中间随宽度自适应
实现要点：
...
这里用 `display: table` 和 `display: block` 在大部分情况下是等价的，只有当需要清除浮动的元素（类为 clearfix 的元素）内容为空时产生区别：由于前者可以产生 BFC，上下方元素边距不会合并。
